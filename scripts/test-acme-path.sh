...
HTTP_CODE=$(curl -s -o /dev/null -w "%{http_code}" "http://api.thamquannhan.io.vn/.well-known/acme-challenge/test.txt" 2>/dev/null || echo "000")
if [ "$HTTP_CODE" = "200" ]; then
    echo "✓ Domain trả về HTTP 200"
    echo "Nội dung:"
    curl "http://api.thamquannhan.io.vn/.well-known/acme-challenge/test.txt" 2>/dev/null
else
