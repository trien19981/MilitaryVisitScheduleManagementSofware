...
- Domain `api.thamhoi.io.vn` cần truy cập được qua port 80/443 chuẩn
...
```bash
sudo nano /etc/nginx/sites-available/api.thamhoi.io.vn
```
...
server {
    ...
    ssl_certificate /etc/letsencrypt/live/api.thamhoi.io.vn/fullchain.pem;
    ssl_certificate_key /etc/letsencrypt/live/api.thamhoi.io.vn/privkey.pem;
...
```bash
sudo ln -s /etc/nginx/sites-available/api.thamhoi.io.vn /etc/nginx/sites-enabled/
...
```bash
sudo nano /etc/apache2/sites-available/api.thamhoi.io.vn.conf
```
...
<VirtualHost *:443>
    ...
    SSLEngine on
    SSLCertificateFile /etc/letsencrypt/live/api.thamhoi.io.vn/fullchain.pem
    SSLCertificateKeyFile /etc/letsencrypt/live/api.thamhoi.io.vn/privkey.pem
...
```bash
sudo a2ensite api.thamhoi.io.vn.conf
...
1. **Kiểm tra port 80 có thể truy cập được:**
   ```bash
   curl -I http://api.thamhoi.io.vn/api/health
   ```

2. **Kiểm tra từ bên ngoài:**
   ```bash
   curl http://api.thamhoi.io.vn/.well-known/acme-challenge/test
   ```
...
- Test: `curl http://api.thamhoi.io.vn/.well-known/acme-challenge/test`
